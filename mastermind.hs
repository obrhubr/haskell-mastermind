...
getComparison array1 array2 = (blackPins, whitePins)
    where   blackPins = getBlackPins array1 array2
...
generateSeq :: [[Int]]
generateSeq = [[a,b,c,d,e] | a <- [1..8], b <- [1..8], c <- [1..8], d <- [1..8], e <- [1..8]]

checkNewSeqOldSeq :: [Int] -> ((Int, Int), [Int]) -> Int
checkNewSeqOldSeq a ((bp, wp), b) = if getComparison a b == (bp, wp) then 0 else 1
